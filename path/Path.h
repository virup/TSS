...
namespace stu{
    ...
    class Path
    {
        ...
        public:
            bool isBO(); // return true, if path points to BO
            bool isSO(); // return true, if path points to SO
            bool isRO(); // return true, if path points to RO
            bool isList(); // return true, if path points to List
            ...
            // Extends the path
            // eg. Path p2 = p1 + "leftEndPoint"
            Path& operator+(string p);

            // Assigns a Path to another Path object
            Path& operator=(const Path & path);

            //Vector encapsulates pathstring + access code
    ...
    ////////////////////////////////////////////////////////////////////
    template<class T> int Path::set(T &val)
    {
        // This Path should NOT be a list
        if (this->isList())
    ...
    template<class T> int Path::set(T *val, int size)
    {
        Locator l;

        // This Path SHOULD be a list
        if(!this->isList())
    ...
    template<class T> uint Path::get(T &val)
    {
    ...
    template<class T> uint Path::get(T *&val, int &size)
    {
        if(!this->isConsistent())
            this->makeConsistent();

        // Should be RO or BO and a list
    ...
    template<class T> int Path::append(T *val, int size)
    {
        // Should be a List and a BO
        if(!this->isList())
